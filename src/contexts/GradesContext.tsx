...
export const GradesProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchGrades = async () => {
    ...
    const { data, error } = await supabase
      .from('grades')
      .select('id, student_id, module_id, assignment1, assignment2, midterm, final, overall, created_at, updated_at, students:student_id(first_name, last_name, student_id, filiere, level), modules:module_id(code, name, filiere, academic_level, semester)');
